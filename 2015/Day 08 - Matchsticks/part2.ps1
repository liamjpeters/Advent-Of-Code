...
foreach ($line in $rawInput) {
    ...
    # Start by adding a double quote
    $builder.Append('"') | Out-Null

    # Go letter by letter and build the new encoded string
    foreach ($letter in $line.ToCharArray()) {
        # If the letter is a \ or ", then add that prepended with a `\`
        if ($letter -in @('\','"')) {
            $builder.Append("\$($letter)") | Out-Null
        } else {
    ...
    # End by adding a double quote
    $builder.Append('"') | Out-Null

    $parsedString.encodedString = $builder.ToString()

    $parsedStrings += $parsedString
}

$TotalEncodedStringLength = $parsedStrings.encodedString | ForEach-Object {
    $_.Length
...
$TotalEncodedStringLength - $TotalCodeStringLength
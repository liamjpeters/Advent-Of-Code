...
# Track the number of increases
$increases = 0

# Loop starting at the second element so we can always safely reference the
# previous element
for ($i = 1; $i -lt $depthMeasurements.Count; $i++) {
    if ($depthMeasurements[$i] -gt $depthMeasurements[$i-1]) {
        $increases++
    }
}
$increases
...
# Read in all numbers as integers
$depthMeasurements = $rawInput | ForEach-Object { [int32]$_ }

# Keep track of increases
$increases = 0

# Initilise the 'previous' window to the sum of the first 3 digits
$previousWindow = $depthMeasurements[0..2] |
    Measure-Object -Sum |
    Select-Object -Expand Sum

# Loop starting at the fourth element, so we can safely reference the previous
# 2 elements, as well as having pre-calculated the first 3 window sum.
for ($i = 3; $i -lt $depthMeasurements.Count; $i++) {
    $currentWindow = $depthMeasurements[($i-2)..$i] |
        Measure-Object -Sum |
        Select-Object -Expand Sum

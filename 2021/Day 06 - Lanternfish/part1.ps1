...
$fishArray = $rawInput -split ',' | ForEach-Object { [int]$_ }
$fishList = [System.Collections.ArrayList]::new($fishArray)

foreach ($day in 1..80) {
    for ($i = $fishList.Count - 1; $i -ge 0; $i--) {
        if ($fishList[$i] -eq 0) {
            $fishList[$i] = 6
            $fishList.Add(8) | Out-Null
        } else {
            $fishList[$i]--
        }
    }

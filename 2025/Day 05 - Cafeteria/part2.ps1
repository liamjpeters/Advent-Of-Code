...
Import-Module "$PSScriptRoot\..\..\_modules\Ranges.psm1" -Force
...
$readingRanges = $true
# Read the ranges
:RangeLoop foreach ($line in $rawInput) {
    if ($line -eq '') {
        break RangeLoop
    }
    if ($readingRanges) {
        $start, $stop = $line -split '-'
        $freshRanges += [PSCustomObject]@{
            Start = [int64]$start
            End   = [int64]$stop
        }
    }
...
# Merge the ranges down to non-overlapping ranges
$freshRanges = Merge-Ranges -Ranges $freshRanges

$rangeSizeSum = 0
foreach ($range in $freshRanges) {
    $rangeSizeSum += ($range.End - $range.Start + 1)
}
$rangeSizeSum

...
$splitRanges = $rawInput -split ','

$invalidSum = 0

foreach ($range in $splitRanges) {
    $bounds = $range -split '-'
    $start = [int64]$bounds[0]
    $end = [int64]$bounds[1]
    ...
    for ($i = $start; $i -le $end; $i++) {
        ...
        for ($subLen = 1; $subLen -le [math]::Floor($length / 2); $subLen++) {
            if ($length % $subLen -eq 0) {
                $subStr = $idStr.Substring(0, $subLen)
                $repeatedStr = ($subStr * ($length / $subLen))

                if ($repeatedStr -eq $idStr) {
                    $invalidSum += $i
                    break
                }
            }

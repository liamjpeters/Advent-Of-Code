...
# Parse instructions into something meaningful
...
foreach ($instruction in $instructions) {
    # Determine the sign of the rotation. Left is moving more negative, right is
    # more positive
    $sign = if ($instruction.Direction -eq 'L') {
        -1
    } else {
        1
    }
    # Simulate each click of the dial
    for ($click = 1; $click -le $instruction.Distance; $click++) {
        # Increment the current position one step at a time
        $currentPosition += $sign

        # Wrap current position so it's never negative or >= 100
        $currentPosition = ($currentPosition % 100 + 100) % 100

        # Check if we hit zero during the rotation

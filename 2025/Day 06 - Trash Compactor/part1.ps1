...
foreach ($line in $rawInput) {
    $parts = $line -split '\s+' | Where-Object {
        $_ -ne ''
    }
    for ($i = 0; $i -lt $parts.Count; $i++) {
        if ($i -ge $problems.Count) {
        ...
        if ($parts[$i] -in @('+','*')) {
            $problems[$i].Operation = $parts[$i]
        } else {
            $problems[$i].Numbers += $parts[$i] -as [int]
        }
    }
}
...
foreach ($problem in $problems) {
    if ($problem.Operation -eq '+') {
        $answer = 0
        foreach ($number in $problem.Numbers) {
            $answer += $number
        }
    } elseif ($problem.Operation -eq '*') {
        $answer = 1
        foreach ($number in $problem.Numbers) {
            $answer *= $number
        }
